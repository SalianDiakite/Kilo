-- Migration to update get_conversations_for_user RPC
-- to calculate unread_count based on conversation_participants.last_read_at
...
BEGIN
  ...
  SELECT
    ...
    (
      SELECT COUNT(m.id)
      FROM messages m
      JOIN conversation_participants cp_user ON m.conversation_id = cp_user.conversation_id AND cp_user.user_id = p_user_id
      WHERE m.conversation_id = c.id
        AND m.sender_id != p_user_id -- Only count messages from the other participant
        AND m.created_at > cp_user.last_read_at -- Message created after user last read
    ) AS unread_count,
    t.id AS trip_id,
